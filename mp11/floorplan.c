...
// In this MP, we wrote functions that helped us with model the floorplan of a circuit
// We used trees with horizontal and vertical cutslices in addition to modules (which
// are numbered) to model the physical representation of the floorplan as a tree
// We wrote different functions that helped us create the desired trees but also
// manipulate them performing operations such as rotation and also counting the
// resources that are allocated for each module
...
int is_leaf_node(node_t* ptr) {
 ...
 if ((ptr->left == NULL) && (ptr->right==NULL)) { // check if leaf node
   return 1; // is leaf node
 }
 return 0; // is not leaf node
}
...
int is_internal_node(node_t* ptr) {
 // TODO: (remember to modify the return value appropriately)
 if ((ptr->left != NULL) && (ptr->right != NULL)) { // check if not leaf node
   return 1;
 }
 return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
 ...
 if(a == NULL || b == NULL) // Check if null
  return 0;
if(a==b) { // Check if equivalent
  return 1;
}
 return is_in_subtree(a->right, b) || is_in_subtree(a->left, b); // Traverse binary tree to try make equivalent
...
void rotate(node_t* ptr) {
 // TODO:
 // Use temp variable to swap w and h
...
void swap_module(node_t* a, node_t* b) {
 ...
 // TODO:
 // Use temporary variable to swap modules from a and b
...
void swap_topology(node_t* a, node_t* b) {
 ...
 // TODO:
 // save temp parent nodes for later
 node_t *a_par = a->parent;
 node_t *b_par = b->parent;
 if(a->parent->left == a) { // a is in left
  if(b->parent->left == b) { // b is in left
    // both a and b on left sides
    a->parent->left = b;
    b->parent->left = a;
  } else {
 ...
 // use old parent variables to swap back
 b->parent = a_par;
 a->parent = b_par;
 return;
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
 ...
 //expression is pointer to array to fill like below with data from ptr
 if(ptr->module != NULL) {
...
int get_total_resource(node_t* ptr)
{
 ...
 if(ptr->left == NULL && ptr->right == NULL) { //if leaf node, guarenteed not cutline
    return ptr->module->resource; // return individual value
 }
 return get_total_resource(ptr->left) + get_total_resource(ptr->right); // resursivly traverse, add, and return at the same time
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  //Base case
  if(n == num_modules - 1) {
    // assign data to the module pointed by ptr
  ...
  ptr->right = newNode; //malloc new node
